...
void MetodoSacudida(std::vector<int> v){
	...
	if(!estaOrdenado(v)){
		...
		std::cout<<"Error en el algoritmo\n";
		exit(-1);
	}
}
...
void realizarMetodo(int tipoMetodo){
	...
	int numeroTiempos = (data.max_valor-data.min_valor)/data.incremento;
	...
	escribirFichero("Datos.txt", n, tiempos, tiemposMinimosCuadrados);
