...
cv::Mat extractSURFDescriptors(const cv::Mat& img, const int threshold)
{
...
cv::Mat
compute_bovw (const cv::Ptr<cv::ml::KNearest>& dict, const int dict_size, cv::Mat& img_descs, bool normalize)
{
